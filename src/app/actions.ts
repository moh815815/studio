...
import { AIServiceSearch, type AIServiceSearchInput, type AIServiceSearchOutput, type AIServiceRecommendation } from '@/ai/flows/ai-service-search';
...
type SearchState = {
    message: string | null;
    results: AIServiceRecommendation[];
    error: boolean;
}
...
export async function searchServicesAction(
  ...
  try {
    ...
    const aiResponse = await AIServiceSearch(input);
    return { message: aiResponse.message, results: aiResponse.results, error: false };
  } catch (error) {
